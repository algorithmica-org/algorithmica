...
void fft(vector<ftype> &p, ftype wn) {
    ...
    ftype w = 1;
    for (int i = 0; i < n / 2; i++) {
        // можно не использовать модуло, а сразу раскрыть его для двух половин
        p[i] = a[i] + w * b[i];
...
Если мы знаем, где окажется каждый элемен, то давайте тогда даже не будем делать какие-либо перестановки внутри рекурсии, а просто выполним их один раз в самом начале, а в рекурсивной функции будем просто запускаться от половин.
...
void solve(ftype *a, int n, ftype wn) {
    if (n > 1) {
        ...
        for (int i = 0; i < k; i++) {
            ...
            // потому что мы читаем и пишем из одного массива
            ftype t = w * a[i + k];
            a[i + k] = a[i] - t; 
            a[i] = a[i] + t;
            w *= wn;
...
void fft(ftype *a, int n, int inverse) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int l = 0; l < logn; l++)
            k |= ((i >> l & 1) << (logn - l - 1));
        // делаем только один свап -- из того элемента, который идет раньше
        if (i < k)
            swap(a[i], a[k]);
    }
