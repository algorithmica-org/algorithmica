...
- `Forward Iterator` требует, помимо предыдущей категории, то, что итераторы на какой-то конкретный элемент можно инкрементировать сколько угодно раз, но его копии останутся валидными (что позволяет их использовать в алгоритмах, проходящих по данным несколько раз). Также доступен постфиксный инкремент.
