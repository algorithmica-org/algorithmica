...
Само решение максимально простое: отсортируем вершины графа по их степени, будем проверять только ребра $v \rightarrow u, v \le u$; теперь внутренним циклом будем перебирать пути $v \rightarrow u \rightarrow w, v \le u \le w$, а потом проверять существование ребра $v \rightarrow w$.
...
for (int v : p) {
    ...
    for (int u : g[v]) {
        if (cmp(v, u)) break;
        for (int w : g[u]) {
            if (cmp(u, w)) break; // Если ребро плохое - не берем треугольники
